...
> `0.1.0-alpha.1` is an evaluation release, not a production-ready declaration. Check the [alpha support matrix](docs/public/support-matrix.md) and [stability policy](docs/public/stability.md) before production use.
...
Alpha releases use the npm `alpha` dist-tag. Run this only after `0.1.0-alpha.1` appears in [npm package versions](https://www.npmjs.com/package/better-realtime?activeTab=versions). An npm `E404` means the approval-gated publication has not completed; it does not reserve the name.
...
| `standalone-memory` | not-applicable | internal-fixture | not-planned | repository test harness only |
| `socket-io` | not-defined | not-implemented | committed-post-alpha | — |
| `telemetry-contract` | not-defined | not-implemented | committed-post-alpha | — |
| `sentry` | not-applicable | not-implemented | committed-post-alpha | — |
| `opentelemetry` | not-applicable | not-implemented | committed-post-alpha | — |
| `react-native` | not-applicable | not-implemented | committed-post-alpha | — |
| `go-server` | not-applicable | not-implemented | committed-post-alpha | — |
| `durable-diagnostics` | not-applicable | not-implemented | committed-post-alpha | — |
| `production-mcp` | not-applicable | not-implemented | committed-post-alpha | — |
| `presence` | not-defined | not-implemented | demand-gated-candidate | — |
