...
The vendored artifact can be independently refreshed only from its recorded registry URL:

```sh
pnpm compatibility:acquire -- --verify-external
```

The command refuses byte drift. Replacing or deleting the alpha.1 fixture is not a supported workflow.
