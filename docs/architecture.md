...
Client and server facades optionally export those existing facts through the same browser-safe `EvidenceSink` structure. A bounded synchronous-to-asynchronous bridge owns redaction, producer declaration, queue capacity, flush status, and exact close checkpoints; the application owns the durable ingestion destination and tenant authorization. `DiagnosticSource` is a separate query boundary, and production MCP can only sit behind an authenticated, authorized, audited source. A gateway never exposes its recorder or local MCP directly as a public endpoint.
...
`stateStream()` is the normal state-materialization path. It keeps transport sequence and snapshot-fencing metadata in a framework-owned snapshot envelope, co-locates each event payload with its precisely inferred reducer, and declares `state_reducer_v1` in the manifest. `stream()` remains the low-level escape hatch for custom materializers and explicit snapshot metadata. Both emit the same language-neutral manifest model; reducer functions never become part of the wire contract.
...
- State reducer: `stateStream()` applies a typed event-local reducer while the framework owns the included sequence.
...
The public `better-realtime` tarball packages that graph behind browser-safe root and `/react` exports. `/server` and `/diagnostics` are explicitly Node-only and resolve to an agent-readable failure module under the browser condition. PostgreSQL and `ws` are optional server peers, so a browser-only installation does not install them. The local stdio analyzer is the separately publishable `better-realtime-mcp` companion and depends on the base diagnostics contract; MCP SDK and its transport dependency graph do not enter a browser or ordinary server installation. The server facade accepts a contract, PostgreSQL profile, authentication callback, typed stream authorization/snapshot callbacks, and typed framework-owned command execution callbacks. The diagnostic facade separates source evidence from the recursively closed, pseudonym-only public result schema. CLI and MCP reuse one analyzer, and MCP can expand only a doctor-issued bounded evidence closure rather than accepting public pseudonyms as fresh raw query identifiers. `PostgresGatewayServer`, `PostgresEventLog`, transport factories, transaction state machines, and recorder internals remain internal ports.
...
The normal command path calls `targets(input)` before opening the application transaction, sorts and locks those stream targets deterministically, then invokes `execute(ctx, input, tx)`. The lifetime-bounded transaction port permits application queries and zero or more `tx.emit()` calls only to declared targets. Result, events, outbox rows, projections, and application writes commit in the same framework-owned transaction. Transaction control, DDL, session advisory locks, raw clients, and emits outside the declared target set are rejected. The versioned canonical intent binds command/input identity and the complete ordered target set, while attempt identity remains distinct from stable command identity. The alpha.4 `prepare` plus `mutate` path is confined to a compatibility adapter and retains its single-event behavior. Neither path permits a best-effort publish escape hatch; a retry with a changed intent is rejected instead of producing a second effect.

PostgreSQL storage v2 adds the command-to-zero-or-many-event relation through an immutable deployment migration. Runtime startup validates the installed version and never performs DDL. `better-realtime.v1` already carries a bounded causal event vector, so this storage/API evolution requires a package and database version boundary but no wire-major change.

The reference server currently implements standalone `start()` ownership. An attach/upgrade API for Fastify, Nest, or another existing HTTP server remains unclaimed until listener ownership, path routing, Origin/auth parity, capacity/backpressure, and drain behavior have the same executable conformance evidence. A generic raw upgrade hook is not accepted as a shortcut around those guarantees.
