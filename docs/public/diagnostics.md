...
`better-realtime doctor`, `trace command`, `inspect stream`, `leaks`, and MCP v0 all use the same deterministic query/analyzer API. The `0.2` release moves MCP to the separately installed `better-realtime-mcp` package. Current MCP is an offline, stdio, read-only analyzer for a local evidence bundle explicitly extracted by the application. It rejects URL sources, bounds input and pagination, preserves tenant scope, and does not require sensitive payload capture.
