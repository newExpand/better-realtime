...
Better Realtime 0.1.0-alpha.1 supports React Web clients and Node.js servers using PostgreSQL.

```sh
npm install better-realtime@alpha react pg ws
```

Alpha releases use the npm `alpha` dist-tag. Run the command only after `0.1.0-alpha.1` appears in [npm package versions](https://www.npmjs.com/package/better-realtime?activeTab=versions); an `E404` means the approval-gated publication has not completed.
