...
Protocol version, package SemVer, contract manifest version/digest, and payload schema identity are separate. Package versions on `better-realtime.v1` may interoperate only when exact contract identity and the capability profile required by the deployment agree. Contract mismatches still require a coordinated deployment or versioned host/path and return `RT_CONTRACT_INCOMPATIBLE`; an internally contradictory capability set returns `RT_CAPABILITY_VIOLATED`. Protocol v1 does not carry client-declared capability requirements in `session.open`, so a valid but weaker profile is not a generic handshake error: applications must select a compatible endpoint/profile and must not infer guarantees that the server did not advertise. Adding generic client-declared requirements is a protocol-version decision, not a silent v1 reinterpretation. Optional additive fields are v1-compatible only when old peers can safely ignore them and no side infers a stronger guarantee. An incompatible frame or semantic change uses `better-realtime.v2`, with any old-peer conversion isolated in an explicit translator rather than package-version branches inside core state machines.
...
A client MUST NOT infer a stronger guarantee than the declared capabilities. Independent capability fields and parameters, not a product-level numeric level, are normative. Protocol 1.0 advertises a session profile but has no wire field through which a client declares a required profile. An implementation may reject an operation with a structured error when that operation cannot be honored, but it MUST NOT describe a valid weaker session as having negotiated requirements that were never sent.
