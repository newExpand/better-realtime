...
describe("read-only diagnostic CLI and MCP", () => {
  it("returns the same proven derived incident result through CLI and MCP", async () => {
    ...
    expect(JSON.parse(invalidContent[0]?.type === "text" ? invalidContent[0].text ?? "null" : "null")).toEqual({ product: "Better Realtime", productVersion: "0.1.0-alpha.3", component: "better-realtime", schemaVersion: "1.0", kind: "diagnostic_error", code: "RT_DIAGNOSTIC_QUERY_INVALID", message: "RT_DIAGNOSTIC_QUERY_INVALID" });
    await client.close();
    await server.close();
  });
  ...
  it("keeps the raw bundle, key, and path behind an opaque source and sanitizes file failures", async () => {
    ...
    expect(JSON.parse(missingErrors.join(""))).toEqual({ product: "Better Realtime", productVersion: "0.1.0-alpha.3", component: "better-realtime", schemaVersion: "1.0", kind: "diagnostic_error", code: "RT_DIAGNOSTIC_SOURCE_UNAVAILABLE", message: "RT_DIAGNOSTIC_SOURCE_UNAVAILABLE" });
  ...
  it("returns the same principal-scope refusal through CLI and MCP", async () => {
    ...
    const content = response.content as Array<{ type: string; text?: string }>;
    expect(JSON.parse(content[0]?.type === "text" ? content[0].text ?? "null" : "null")).toEqual({ product: "Better Realtime", productVersion: "0.1.0-alpha.3", component: "better-realtime", schemaVersion: "1.0", kind: "diagnostic_error", code: "RT_DIAGNOSTIC_SCOPE_AMBIGUOUS", message: "RT_DIAGNOSTIC_SCOPE_AMBIGUOUS" });
    await client.close();
    await server.close();
  });
});
