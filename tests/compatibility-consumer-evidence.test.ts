...
describe("external consumer diagnostic closure", () => {
  ...
  it("rejects cross-version capability drift and unstable retry outcomes", () => {
    ...
    const profiles = ["alpha1-client-to-candidate-server", "candidate-client-to-alpha1-server", "candidate-client-to-candidate-server"].map((id) => ({ id, capabilityProfile: capability }));
