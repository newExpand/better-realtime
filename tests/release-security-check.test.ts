...
describe("release-security contract modes", () => {
  ...
  it("fails closed on state-machine, numeric-ID, and duplicate-publish mutations", async () => {
    ...
    const publishMutations = [
      ...
      publish.replace(`test "$(jq -sr '[.[].name] | unique | length' publish-aborts.jsonl)" = "$abort_count"`, "true"),
      publish.replace('test "$active_intent_count" -le 1', 'test "$active_intent_count" -le 99'),
      publish.replace("ARTIFACT: ${{ github.workspace }}/release-assets/${{ needs.build.outputs.artifact }}", "ARTIFACT: release-assets/${{ needs.build.outputs.artifact }}"),
    ...
    const adapterMutations = [
      ...
      adapter.replaceAll("resolvePublishIntentLedger", "ignorePublishIntentLedger"),
      adapter.replaceAll("assertLocalArtifactSpecFailureEvidence", "ignoreLocalArtifactSpecFailureEvidence"),
      adapter.replaceAll("actions/jobs/${publishJobs[0]!.id}/logs", "actions/jobs/ignored/logs"),
      adapter.replaceAll("recover-local-artifact-spec-failure", "ignore-local-artifact-spec-failure"),
